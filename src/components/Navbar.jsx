    ...
    <motion.header initial={{ y: -60 }} animate={{ y: 0 }} transition={{ duration: .55 }} className="fixed inset-x-0 top-0 z-50">
      <nav className={`transition-all duration-300 ${scrolled ? "border-b border-ink-100/70 bg-white/90 shadow-soft backdrop-blur-xl" : "bg-white/70 backdrop-blur-sm"}`}>
        <div className="site-container flex h-[76px] items-center justify-between">
          <a href="#home" className="flex items-center gap-2.5" aria-label="SmileCare home">
            <span className="grid h-10 w-10 place-items-center rounded-2xl bg-aqua-600 text-white shadow-soft">
              <span className="font-display text-lg font-extrabold">S</span>
            </span>
            <span className="font-display text-xl font-extrabold tracking-tight text-ink-950">Smile<span className="text-aqua-600">Care</span></span>
          </a>
          ...
          <a href="tel:+917306674866" className="btn-primary hidden px-5 py-2.5 text-sm lg:inline-flex"><Phone className="h-4 w-4" />Book a visit</a>

          <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open} aria-label={open ? "Close" : "Menu"} className="grid h-10 w-10 place-items-center rounded-xl bg-ink-50 text-ink-800 lg:hidden">
        ...
        <AnimatePresence>
          {open && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-ink-100 bg-white lg:hidden">
            <div className="site-container space-y-1 py-4">
              ...
              <a href="tel:+917306674866" onClick={() => setOpen(false)} className="btn-primary mt-3 w-full"><Phone className="h-4 w-4" />Book a visit</a>
            </div>
          </motion.div>}
        </AnimatePresence>
      </nav>
